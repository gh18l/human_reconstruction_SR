...
def nonrigid_estimation():
    ...
    # cv2.imshow("1", result)
    # cv2.imshow("2", result_weights)
    # cv2.waitKey()

    contours_smpl_index_pairs = []
    contours, hierarchy = cv2.findContours(HR_masks[ind], cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    for i in range(len(contours[17])):
        distances = get_distance(contours[17][i, :, :], uv_real)
        min_smpl_index = np.argmin(distances)
        contours_smpl_index_pairs.append(min_smpl_index)

    ### start optimization
    smpl_contours = np.zeros([len(contours_smpl_index_pairs), 3])
    for i in range(len(contours_smpl_index_pairs)):
        smpl_contours[i, :] = v_final[0][contours_smpl_index_pairs[i], :]
    smpl_contours_tf = tf.Variable(smpl_contours, dtype=tf.float32)
    cam_nonrigid = Perspective_Camera(cam_HR1[0], cam_HR1[0], hmr_cam[1],
                                hmr_cam[2], cam_HR1[3], np.zeros(3))
    verts_est = cam_nonrigid.project(tf.squeeze(smpl_contours_tf))

    objs_norigid = {}
    objs_norigid['verts_loss'] = 1.0 * tf.reduce_sum(tf.square(verts_est - contours[17].squeeze()))
    objs_norigid['smooth_loss'] = 1.0 * tf.reduce_sum(tf.square(smpl_contours_tf - smpl_contours))
    loss = tf.reduce_mean(objs_norigid.values())
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        # L-BFGS-B
        optimizer = scipy_pt(loss=loss, var_list=[smpl_contours_tf],
                             options={'eps': 1e-12, 'ftol': 1e-12, 'maxiter': 1000, 'disp': False}, method='L-BFGS-B')
        optimizer.minimize(sess)
        smpl_contours_final = sess.run(smpl_contours_tf)
        _objs_norigid = sess.run(objs_norigid)
        print("the verts_loss is %f" % _objs_norigid['verts_loss'])
        print("the smooth_loss is %f" % _objs_norigid['smooth_loss'])

    ### assemble new verts
    for i in range(len(contours_smpl_index_pairs)):
        v_final[0][contours_smpl_index_pairs[i], :] = smpl_contours_final[i, :]
    ...
    camera.write_obj(util.hmr_path + "output_nonrigid/hmr_optimization_rotation_nonrigid_%04d.obj" % ind, v_final[0], vt)
nonrigid_estimation()