...
def main(flength=2500.):
    ...
    '''
    model = _load_model(util.SMPL_PATH)
    path = "/home/lgh/code/SMPLify_TF/test/test_hmr_init/tianyiHR/"
    ...
    for ind, HR_j2d in enumerate(HR_j2ds):
        ...
        cam_HR1 = sess.run([cam_HR.fl_x, cam_HR.cx, cam_HR.cy, cam_HR.trans])
        verts_est1 = sess.run(verts_est)
        param_shape1 = sess.run(param_shape)
        print(param_shape1)
        v1 = sess.run(v)
        j3ds1 = sess.run(j3ds)
        jointsplus1 = sess.run(jointsplus)
        camera = render.camera(cam_HR1[0], cam_HR1[1], cam_HR1[2], cam_HR1[3])
        _, vt = camera.generate_uv(v1, HR_imgs[ind])
        img_result_texture = camera.render_texture(v1, HR_imgs[ind], vt)
        cv2.imwrite(path + "output/hmr_optimization_texture_%04d.png" % ind, img_result_texture)
        img_result_naked = camera.render_naked(v1, HR_imgs[ind])
        cv2.imwrite(path + "output/hmr_optimization_%04d.png" % ind, img_result_naked)
        img_result_naked_rotation = camera.render_naked_rotation(v1, 90, HR_imgs[ind])
        cv2.imwrite(path + "output/hmr_optimization_rotation_%04d.png" % ind, img_result_naked_rotation)
        ...
        # ax.scatter(hmr_joint3d[0:14, 0], hmr_joint3d[0:14, 1], hmr_joint3d[0:14, 2], c='b')
        # ax.scatter(jointsplus1[:, 0], jointsplus1[:, 1], jointsplus1[:, 2], c='r')
        # #hmr_joint3d = hmr_joint3ds[ind,:,:]
        ...
        objs['J2D_Loss'] = 0.0 * tf.reduce_sum(weights * tf.reduce_sum(tf.square(j2ds_est - HR_j2d), 1))

        pose_diff = tf.reshape(param_pose - pose_mean, [1, -1])
        objs['Prior_Loss'] = 0.0 * tf.squeeze(tf.matmul(tf.matmul(pose_diff, pose_covariance), tf.transpose(pose_diff)))
        objs['Prior_Shape'] = 0.0 * tf.reduce_sum(tf.square(param_shape))
        ##############################################################
        ...
        objs["angle_elbow_knee"] = 0.0 * tf.reduce_sum(w1 * [
        ...
        objs["arm_leg_direction"] = 0.0 * tf.reduce_sum(tf.exp(-j3ds[6, 2]))
        objs["arm_leg_amplitude"] = 0.0 * tf.reduce_sum(tf.square(j3ds[6,2]+j3ds[11,2]) + tf.square(j3ds[7,2]+j3ds[10,2]))
        ##############################################################
        ###################mask obj###################################
        ##############################################################
        objs['mask'] = 0.0 * tf.reduce_sum(verts2dsilhouette / 255.0 * (255.0 - HR_mask) / 255.0
        ...
        with tf.Session() as sess:
            ...
            optimizer.minimize(sess)
            v_final = sess.run([v, verts_est, j3ds, param_shape])
            cam_HR1 = sess.run([cam_HR.fl_x, cam_HR.cx, cam_HR.cy, cam_HR.trans])
            camera = render.camera(cam_HR1[0], cam_HR1[1], cam_HR1[2], cam_HR1[3])
            ...
            print("the HR face loss is %f" % sess.run(objs['face']))
            if ind != 0:
