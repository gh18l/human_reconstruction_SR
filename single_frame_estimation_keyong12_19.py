...
import util
from camera_naomal import Perspective_Camera
...
def main(flength=2500.):
    ...
    pose_prior_array = np.zeros([len(HR_imgs), 69])
    ...
    #initial_param, pose_mean, pose_covariance = util.load_hr_means_covariance_lsw()
    #pose_mean = np.zeros(69)
    ...
    HR_init_param_rot = flipped_rot.reshape([1, -1])
    LR_init_param_rot = flipped_rot.reshape([1, -1])
    HR_init_param_pose = initial_param[13:82].reshape([1, -1])
    # HR_init_param_pose = np.zeros([1, 69])
    # HR_init_param_pose[0, 50] = 0.86   #1.27
    # HR_init_param_pose[0, 49] = 0.23  #0.52
    # HR_init_param_pose[0, 48] = 0.5   #0.067
    ...
    for ind, HR_j2d in enumerate(HR_j2ds):
        ...
        param_rot = tf.constant(HR_init_param_rot, dtype=tf.float32)
        param_pose = tf.Variable(HR_init_param_pose, dtype=tf.float32)
        param_trans = tf.constant(HR_init_param_trans, dtype=tf.float32)
        param = tf.concat([param_shape, param_rot, param_pose, param_trans], axis=1)
        cam_HR.tran = tf.Variable(cam_HR_init_trans, dtype=tf.float32)
        ...
        # ax.scatter(j3ds1[0, 0], j3ds1[0, 1], j3ds1[0, 2], c='r')
        ...
        objs['Prior_Loss'] = 5.0 * tf.squeeze(tf.matmul(tf.matmul(pose_diff, pose_covariance), tf.transpose(pose_diff)))
        ...
        objs["arm_leg_direction"] = tf.reduce_sum(tf.cond(j3ds[0,2] + j3ds[1,2] < j3ds[4,2] + j3ds[5,2],
                lambda: 0.0 * tf.exp(j3ds[10,2] + j3ds[11,2]),
                    lambda: 0.0 * tf.exp(j3ds[6,2] + j3ds[7,2])))
        #objs["arm_leg_direction"] = tf.reduce_sum(0.0 * tf.exp(j3ds[10, 2] + j3ds[11, 2]))
        ...
        objs['mask'] = 0.06 * tf.reduce_sum(verts2dsilhouette / 255.0 * (255.0 - HR_mask) / 255.0
                                            + (255.0 - verts2dsilhouette) / 255.0 * HR_mask / 255.0)

        w_temporal = [1.0, 1.5, 2.5, 2.5, 1.5, 1.0, 1.0, 1.5, 2.0, 2.0, 1.5, 1.0, 5.0, 5.0]
        if ind != 0:
        ...
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            #L-BFGS-B
            optimizer = scipy_pt(loss=loss, var_list=[cam_HR.trans, param_pose, param_shape],
            ...
            cam_HR1 = sess.run([cam_HR.fl_x, cam_HR.cx, cam_HR.cy, cam_HR.trans])
            camera = render.camera(cam_HR1[0], cam_HR1[1], cam_HR1[2], cam_HR1[3])
            img_result = camera.render_naked(v_final[0], HR_imgs[ind])
            cv2.imwrite("/home/lgh/code/SMPLify_TF/test/test_hmr_init/dingjianLR/output/optimization_%04d.png" % ind, img_result)
            if ind == 3:
                img_result = camera.render_naked_rotation(v_final[0], 90, HR_imgs[ind])
                cv2.imwrite(
                    "/home/lgh/code/SMPLify_TF/test/test_hmr_init/dingjianLR/output/optimization_rotation_%04d.png" % ind,
                    img_result)
                a = 1
            cam_HR_final = sess.run(cam_HR.trans)
            ...
            print("the HR arm_leg_direction loss is %f" % sess.run(objs["arm_leg_direction"]))
            print("the HR arm_leg_amplitude loss is %f" % sess.run(objs["arm_leg_amplitude"]))
            print("the HR mask loss is %f" % sess.run(objs['mask']))
            if ind != 0:
                print("the HR temporal loss is %f" % sess.run(objs['temporal']))
        ...
        cv2.imwrite(util.base_path + "/HR/output_temp/%04d.png" % ind ,HR_masks[ind])
        ...
        cam_HR_init_trans = camera_t_final_HR
        j3ds_old = v_final[2]
        pose_prior_array[ind, :] = HR_init_param_pose
    ...
    #############################################################
    #initial_param, pose_mean, pose_covariance = util.hr_means_covariance_lsw(pose_prior_array)
    #initial_param, pose_mean, pose_covariance = util.load_hr_means_covariance_lsw()
    #pose_mean = tf.constant(pose_mean, dtype=tf.float32)
    #pose_covariance = tf.constant(pose_covariance, dtype=tf.float32)
    #LR_init_param_pose = initial_param[13:82].reshape([1, -1])
    ...
    for ind, LR_j2d in enumerate(LR_j2ds):
        ...
        param_rot = tf.constant(LR_init_param_rot, dtype=tf.float32)
        ...
        pose_diff = tf.reshape(param_pose - pose_mean, [1, -1])
        ######20
        objs['Prior_Loss'] = util.code_params[util.which_people]["Prior_Loss"] * tf.squeeze(tf.matmul(tf.matmul(pose_diff, pose_covariance), tf.transpose(pose_diff)))
        objs['Prior_Shape'] = util.code_params[util.which_people]["Prior_Shape"] * tf.reduce_sum(tf.square(param_shape))
        ##############################################################
        ##########control the angle of the elbow and knee#############
        ##############################################################
        #w1 = np.array([4.04 * 1e2, 4.04 * 1e2, 57.4*3, 4.78*3])
        w1 = np.array([1.04 * 2.0, 1.04 * 2.0, 57.4 * 50, 57.4 * 50])
        w1 = tf.constant(w1, dtype=tf.float32)
        objs["angle_elbow_knee"] = util.code_params[util.which_people]["angle_elbow_knee"] * tf.reduce_sum(w1 * [
            tf.exp(param_pose[0, 52]), tf.exp(-param_pose[0, 55]),
                tf.exp(-param_pose[0, 9]),tf.exp(-param_pose[0, 12])])

        objs["angle_head"] = util.code_params[util.which_people]["angle_head"] * tf.reduce_sum(tf.exp(-param_pose[0, 42]))

        # objs["rot_direction"] = 1000.0 * tf.reduce_sum([tf.abs(param_pose[0, 1]),
        #     tf.abs(param_pose[0, 2]), tf.abs(param_pose[0, 4]), tf.abs(param_pose[0, 5]),
        #         tf.abs(param_pose[0, 10]), tf.abs(param_pose[0, 11]), tf.abs(param_pose[0, 13]),
        #             tf.abs(param_pose[0, 14])])
        objs["rot_direction"] = util.code_params[util.which_people]["rot_direction"] * tf.reduce_sum([tf.abs(param_pose[0, 1]),
            tf.abs(param_pose[0, 2]), tf.abs(param_pose[0, 4]), tf.abs(param_pose[0, 5])])

        # objs["rot_direction"] = 0.1 * tf.reduce_sum([
        #     tf.exp(param_pose[0, 52]), tf.exp(-param_pose[0, 55]),
        #     tf.exp(-param_pose[0, 9]), tf.exp(-param_pose[0, 12])])
        ...
        ######################not successful##########################
        ##############################################################
        w2 = np.array([1.0 * 1e2, 6.0 * 1e2])
        w2 = tf.constant(w2, dtype=tf.float32)
        # objs["arm_leg_direction"] = tf.reduce_sum(w2 * [tf.exp(param_pose[0, 48] * param_pose[0, 45]),
        #     tf.exp(param_pose[0, 0] * param_pose[0, 3])])
        #objs["arm_direction"] = tf.reduce_sum(w2 * [tf.exp(param_pose[0, 48] * param_pose[0, 45])])
        # objs["arm_direction"] = tf.reduce_sum(9999999.0 * tf.cast(tf.less_equal(tf.divide(param_pose[0, 49], param_pose[0, 46]),
        #     0.0), dtype=tf.float32))
        # objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum(tf.cond(param_pose[0, 49] * param_pose[0, 46] < 0,
        #             lambda: tf.exp(-param_pose[0, 49] * param_pose[0, 46]), lambda: 0.0))
        print(util.code_params[util.which_people]["arm_direction"])
        #objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum(
            #tf.cond(param_pose[0, 48] < param_pose[0, 45],
                    #lambda: tf.exp(param_pose[0, 48] * param_pose[0, 45] * 100.0), lambda: 0.0))
        #objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum(
            #tf.cond(param_pose[0, 48] < param_pose[0, 45],
                    #lambda: tf.exp(-param_pose[0, 45] * 100.0), lambda: tf.exp(-param_pose[0, 48] * 100.0)))
        objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum(
            tf.cond(j3ds[7, 2] < j3ds[10, 2],
                lambda: tf.abs(param_pose[0, 55]), lambda: tf.abs(param_pose[0, 52])))
        # objs["arm_direction"] = 100.0 * tf.reduce_sum([tf.exp(-param_pose[0, 49] * param_pose[0, 46])
        #                                                if tf.less(param_pose[0, 49] * param_pose[0, 46], 0.0)
        #                                                else 0.0])
        #objs["arm_direction"] = 100.0 * tf.reduce_sum(-tf.divide(param_pose[0, 49], param_pose[0, 46]))
        #objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum([tf.exp(param_pose[0, 45] * param_pose[0, 48] * 1000.0)])
        #objs["arm_direction"] = util.code_params[util.which_people]["arm_direction"] * tf.reduce_sum(
            #[tf.exp(param_pose[0, 45] * param_pose[0, 48] * 1000.0)])
        ##############################################################
        ###################mask obj###################################
        ##############################################################
        # dst_type = cv2.cv.CV_DIST_L2 if cv2.__version__[0] == '2' else cv2.DIST_L2
        ...
        objs['mask'] = util.code_params[util.which_people]["mask"] * tf.reduce_sum((verts2dsilhouette / 255.0) * (255.0 - LR_mask) / 255.0
                        + (255.0 - verts2dsilhouette) / 255.0 * LR_mask / 255.0)
        ##################process LR mask########################
        dst_type = cv2.DIST_L2
        LR_mask_in = cv2.distanceTransform(np.uint8(LR_masks[ind]), dst_type, 5) - 1
        LR_mask_in[LR_mask_in < 0] = 0
        LR_mask_in[LR_mask_in > 50] = 50
        LR_mask_out = cv2.distanceTransform(255 - np.uint8(LR_masks[ind]), dst_type, 5)
        LR_mask_out[LR_mask_out > 50] = 50
        w_mask = [1.0, 1.0]   ###[LR_mask_out, LR_mask_in]
        #objs['mask'] = util.code_params[util.which_people]["mask"] * tf.reduce_sum((verts2dsilhouette / 255.0) * LR_mask_out * w_mask[0]
            #+ ((255.0 - verts2dsilhouette) / 255.0) * LR_mask_in * w_mask[1])

        ############temporal stabilization############
        #w_temporal = [1.0, 1.5, 2.5, 2.5, 1.5, 1.0, 1.0, 1.5, 2.0, 2.0, 1.5, 1.0, 2.5, 2.5]
        w_temporal = [1.0, 1.5, 2.5, 2.5, 1.5, 1.0, 1.0, 1.5, 2.0, 2.0, 1.5, 1.0, 5.0, 5.0]
        if bad_flag == 0 and ind != 0:
            objs['temporal'] = util.code_params[util.which_people]["temporal"] * tf.reduce_sum(
                w_temporal * tf.reduce_sum(tf.square(j3ds - j3ds_old), 1))
        loss = tf.reduce_mean(objs.values())
        with tf.Session() as sess:
            sess.run(tf.global_variables_initializer())
            #L-BFGS-B
            optimizer = scipy_pt(loss=loss, var_list=[param_pose, param_shape, cam_LR.trans],
                             options={'eps': 1e-12, 'ftol': 1e-10, 'maxiter': 500, 'disp': False}, method='L-BFGS-B')
            optimizer.minimize(sess)
            ...
            # ax = plt.subplot(111)
            # #ax = fig.add_subplot(111, projection='3d')
            # ax.scatter(uv[:3, 0], uv[:3, 1], c='b')
            # ax.scatter(LR_j2d[:3, 0], LR_j2d[:3, 1], c='r')
            # # plt.savefig("/home/lgh/code/SMPLify_TF/test/temp0/1/LR/test_temp/dot.png")
            # plt.show()
            ...
            print("the arm_direction loss is %f" % sess.run(objs["arm_direction"]))
            print("the LR rot_direction loss is %f" % sess.run(objs["rot_direction"]))
            if bad_flag == 0 and ind != 0:
                print("the LR temporal loss is %f" % sess.run(objs["temporal"]))
            model_f = model_f.astype(int).tolist()
            pose_final, betas_final, trans_final = sess.run(
                [tf.concat([param_rot, param_pose], axis=1), param_shape, param_trans])
        ...
        #pose_final_in_opengl[0, 1] = 0.2
        # if pose_final_in_opengl[0, 51] < pose_final_in_opengl[0, 48]:    ###push ind48
        #     if pose_final_in_opengl[0, 48] > 0:
        #         pose_final_in_opengl[0, 48] = pose_final_in_opengl[0, 51] * 5
        #         pose_final_in_opengl[0, 55] = 0
        #     else:
        #         pose_final_in_opengl[0, 48] = -pose_final_in_opengl[0, 51] * 5
        #         pose_final_in_opengl[0, 55] = 0
        # else:
        #     if pose_final_in_opengl[0, 51] > 0:
        #         pose_final_in_opengl[0, 51] = pose_final_in_opengl[0, 48] * 5
        #         pose_final_in_opengl[0, 58] = 0
        #     else:
        #         pose_final_in_opengl[0, 51] = -pose_final_in_opengl[0, 48] * 5
        #         pose_final_in_opengl[0, 58] = 0
        # if pose_final_in_opengl[0, 51] < pose_final_in_opengl[0, 48]:    ###push ind48
        #     pose_final_in_opengl[0, 48] = np.abs(pose_final_in_opengl[0, 51]) * 5.0
        #     pose_final_in_opengl[0, 55] = pose_final_in_opengl[0, 55] / 2
        # else:
        #         pose_final_in_opengl[0, 51] = np.abs(pose_final_in_opengl[0, 48]) * 5.0
        #         pose_final_in_opengl[0, 58] = pose_final_in_opengl[0, 58] / 2
        ...
        res = {'pose': pose_final_in_opengl, 'betas': betas_final, 'trans': trans_final,
               'f': np.array([flength, flength]), 'rt': np.zeros([3]),
               't': cam_LR_final}
        out_pkl_path = out_ply_path.replace('.ply', '.pkl')
        with open(out_pkl_path, 'wb') as fout:
        ...
        cv2.imwrite(util.base_path + "/LR/output_temp/%04d.png" % (util.img_first_index+
                                    util.first_index + ind), LR_masks[ind])
