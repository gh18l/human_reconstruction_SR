...
def main(flength=2500.):
    ...
    for ind, LR_j2d in enumerate(LR_j2ds):
        ...
        param_shape = tf.Variable(hmr_shape.reshape([1, -1]), dtype=tf.float32)
        param_rot = tf.Variable(hmr_theta[0:3].reshape([1, -1]), dtype=tf.float32)
        # for pose_ind in range(69):
        #     pose_temp = hmr_theta[pose_ind + 3]
        #     pose_temp = tf.Variable(pose_temp.reshape([1, -1]), dtype=tf.float32)
        #     ######1--concat to right   0--concat to down
        #     param_pose = tf.concat([param_pose, pose_temp], 1) if pose_ind != 0 else pose_temp
        param_pose = tf.Variable(hmr_theta[3:72].reshape([1, -1]), dtype=tf.float32)
        param_trans = tf.Variable(hmr_tran.reshape([1, -1]), dtype=tf.float32)
        ####tensorflow array initial_param_tf
        initial_param_tf = tf.concat([param_shape, param_rot, param_pose, param_trans], axis=1)
        ...
        #objs["angle_elbow_knee"] = 0.0 * tf.reduce_sum([
            #tf.maximum(param_pose[0, 52], 0.0), tf.abs(tf.minimum(param_pose[0, 55], 0.0)),
            #tf.abs(tf.minimum(param_pose[0, 9], 0.0)), tf.abs(tf.minimum(param_pose[0, 12], 0.0))])
        ...
        objs['face'] = 500000.0 * tf.reduce_sum(tf.square(hmr_joint3d[14:19] - jointsplus[14:19]))
        ...
        with tf.Session() as sess:
            ...
            optimizer = scipy_pt(loss=loss,
                        var_list=[param_rot, param_trans, param_pose, cam_LR.fl_x, cam_LR.fl_y, cam_LR.cx, cam_LR.cy],
                    options={'eps': 1e-12, 'ftol': 1e-10, 'maxiter': 500, 'disp': False})
            optimizer.minimize(sess)
            ...
            _, vt = camera.generate_uv(v_final[0], LR_imgs[ind])
            ...
            print("the LR arm_leg_direction loss is %f" % sess.run(objs["arm_leg_direction"]))
            print("the LR arm_leg_amplitude loss is %f" % sess.run(objs["arm_leg_amplitude"]))
            print("the LR mask loss is %f" % sess.run(objs['mask']))
            print("the LR face loss is %f" % sess.run(objs['face']))
            if ind != 0:
                print("the LR temporal loss is %f" % sess.run(objs['temporal']))
                print("the LR temporal_pose loss is %f" % sess.run(objs['temporal_pose']))
