...
def initialize_camera(smpl_model,
    ...
    init_camera_r = np.array([0.0, np.pi, 0.0])
    ...
    for j, jdx in enumerate(util.TORSO_IDS):
        ...
        ##########################################################################
        objs["j2d_loss_camera_%d" % j] = tf.reduce_sum(tf.square(j2d_estimation[jdx] - j2d[jdx]))
    ...
    with tf.Session() as sess:
        ...
        optimizer = scipy_pt(loss=loss, var_list=[camera_t],
                     options={'ftol': 0.00000001, 'maxiter': 500, 'disp': True}, method='L-BFGS-B')
...
def main(flength=2500.):
    ...
    for ind, HR_j2d in enumerate(HR_j2ds):
        ...
        objs['Prior_Loss'] = 7 * tf.squeeze(tf.matmul(tf.matmul(pose_diff, pose_covariance), tf.transpose(pose_diff)))
        ...
        objs['mask'] = 0.04 * tf.reduce_sum(verts2dsilhouette / 255.0 * (255.0 - HR_mask) / 255.0
